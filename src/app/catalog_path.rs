use crate::cli::interaction::prompt_use_non_existing_catalog_file;
use std::env;
use std::path::{Path, PathBuf};

use anyhow::{Result, bail};

pub const CATALOG_FILE_ENV_VAR: &str = "ALIASMGR_CATALOG_PATH";

pub fn determine_catalog_path() -> Result<Option<PathBuf>> {
    if let Ok(path) = env::var(CATALOG_FILE_ENV_VAR) {
        let path = PathBuf::from(path);
        handle_catalog_file(&path, prompt_use_non_existing_catalog_file)
    } else {
        Ok(None)
    }
}

fn handle_catalog_file(path: &Path, create: impl Fn(&str) -> bool) -> Result<Option<PathBuf>> {
    if path.exists() {
        return Ok(Some(path.to_path_buf()));
    }

    if create(path.to_str().unwrap()) {
        return Ok(Some(path.to_path_buf()));
    }

    bail!(
        "Catalog file '{}' does not exist and user chose not to use it.",
        path.to_str().unwrap()
    );
}

#[cfg(test)]
mod tests {
    use super::*;
    use temp_env::with_var;

    #[test]
    fn test_determine_catalog_path_env_var_set_existing_file() {
        let temp_file = tempfile::NamedTempFile::new().unwrap();
        with_var(
            CATALOG_FILE_ENV_VAR,
            Some(temp_file.path().to_str().unwrap()),
            || {
                let result = determine_catalog_path().unwrap();
                assert_eq!(result, Some(temp_file.path().to_path_buf()));
            },
        );
    }

    #[test]
    fn test_determine_catalog_path_set_non_existing_file_user_accepts() {
        with_var(
            CATALOG_FILE_ENV_VAR,
            Some("/non/existing/catalog/file"),
            || {
                let result =
                    handle_catalog_file(&PathBuf::from("/non/existing/catalog/file"), |_| true);
                assert!(result.is_ok());
                assert_eq!(
                    result.unwrap(),
                    Some(PathBuf::from("/non/existing/catalog/file"))
                );
            },
        );
    }

    #[test]
    fn test_determine_catalog_path_set_non_existing_file_user_declines() {
        with_var(
            CATALOG_FILE_ENV_VAR,
            Some("/non/existing/catalog/file"),
            || {
                let result =
                    handle_catalog_file(&PathBuf::from("/non/existing/catalog/file"), |_| false);
                assert!(result.is_err());
            },
        );
    }

    #[test]
    fn test_determine_catalog_path_env_var_not_set() {
        with_var(CATALOG_FILE_ENV_VAR, None as Option<&str>, || {
            let result = determine_catalog_path().unwrap();
            assert_eq!(result, None);
        });
    }
}
